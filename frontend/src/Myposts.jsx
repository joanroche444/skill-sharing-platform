...
const Myposts = () => {
  ...
  const [editPost, setEditPost] = useState(null); // State to hold post being edited
  ...
  const handleDelete = async (postid) => {
    ...
    if (!confirmDelete) {
      return; // If user clicks "Cancel", abort the deletion
    }
  ...
  const handleEdit = (postid) => {
    ...
    setEditPost(postToEdit); // Set the post to edit
  ...
  const handleUpdate = async (event) => {
    ...
    const updatedPost = {
        ...
        createdBy: 'james_smith' // Set createdBy statically
      };
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewPost({ title: '', description: '', mediaUrl: '', mediaType: '' }); // Reset the form
      } else {
      ...
      {editPost ? (
        <div className="bg-white rounded-lg shadow-md p-8 max-w-3xl mx-auto">
          ...
          <form onSubmit={handleUpdate} className="space-y-6">
            ...
            <button type="submit" className="w-full bg-indigo-600 text-white py-3 rounded-lg shadow-lg text-lg hover:bg-indigo-700 focus:outline-none focus:ring-4 focus:ring-indigo-500">
              Update Post
            </button>
          </form>
        </div>
